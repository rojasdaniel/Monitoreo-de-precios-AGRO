...
if (rta=="1"){
  for (central in centrales){
    ...
    for (p1 in centr_df){
      for (p2 in centr_df){
        ...
        calculation <- InfNormDistance(jdf[,1], jdf[,2])
    ...
    png(file=paste(central," - Distancia Norma Infinita",".png",sep=""),width = 1000, height = 1000)
    plot <- levelplot(m[1:ncol(m),ncol(m):1],col.regions=new.palette(20), scales=list(x=list(rot=90, cex=1),y=list(cex=1)), xlab=list(cex=0), ylab=list(cex=0),  main = paste(central, "Distancia Norma Infinita",sep="-"))
    print(plot)
    dev.off()
  }
}else{
  for (product in productos){
    ...
    #setwd(file.path(mainDir, subDir))
    new.palette=colorRampPalette(c("black","red","yellow","white"),space="rgb") 
    png(file=paste(product," - Distancia Norma Infinita",".png",sep=""),width = 1000, height = 1000)
    plot <- levelplot(m[1:ncol(m),ncol(m):1],col.regions=new.palette(20), scales=list(x=list(rot=90, cex=1),y=list(cex=1)), xlab=list(cex=0), ylab=list(cex=0),  main = paste(product, "Distancia Norma Infinita",sep="-"))
    print(plot)
    dev.off()
